/**
 * @file src/platform/windows/vhid_bridge.cpp
 * @brief virtual HID driver backend (keyboard + mouse).
 */
// platform includes
#include <windows.h>
#include <hidsdi.h>
...
namespace platf::vhid_bridge {
  ...
  namespace {
    // Contract: the driver's report contract (vendor 0x4C43, product 0x0001)
    ...
    /**
     * @brief Whether an open HID handle is the virtual input control collection.
     */
  ...
  std::unique_ptr<device_t> device_t::open() {
    ...
    for (DWORD index = 0; !found && SetupDiEnumDeviceInterfaces(set, nullptr, &guid, index, &data); ++index) {
      ...
      if (is_control_collection(handle)) {
        ...
        BOOST_LOG(info) << "virtual HID driver found; keyboard and mouse use the HID path"sv;
      } else {
        CloseHandle(handle);
      }
    }
  ...
  bool device_t::inject(const std::uint8_t *report, std::size_t length) {
    ...
    if (!WriteFile(static_cast<HANDLE>(handle_), frame, static_cast<DWORD>(sizeof(frame)), &written, nullptr) || written != sizeof(frame)) {
      BOOST_LOG(warning) << "virtual input virtual HID write failed: "sv << GetLastError();
      return false;
    }
    return true;
  }
  ...
  void device_t::key(std::uint16_t vk, bool pressed) {
    ...
    if (usage == 0) {
      BOOST_LOG(debug) << "virtual input virtual HID: unmapped virtual key "sv << vk;
      return;
    }
