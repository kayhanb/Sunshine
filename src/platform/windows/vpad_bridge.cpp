/**
 * @file src/platform/windows/vpad_bridge.cpp
...
#pragma pack(pop)
    ...
    bool open_pad(int index, pad_t &pad) {
      const std::wstring base = L"Global\\virtual inputVpad" + std::to_wstring(index);
      ...
      if (!pad.shared || pad.shared->magic != shared_magic || pad.shared->layout_version != shared_layout_version) {
        BOOST_LOG(warning) << "virtual input pad "sv << index << ": unexpected channel layout, pad skipped"sv;
        close_pad(pad);
        return false;
      }
      ...
      if (!pad.input_event || !pad.rumble_event) {
        BOOST_LOG(warning) << "virtual input pad "sv << index << ": channel events missing ["sv << GetLastError() << ']';
        close_pad(pad);
        return false;
      }
      return true;
  ...
  std::unique_ptr<context_t> open() {
    ...
    BOOST_LOG(info) << "virtual input pad driver: "sv << context->count() << " XInput pad(s) available"sv;
    return context;
  }
